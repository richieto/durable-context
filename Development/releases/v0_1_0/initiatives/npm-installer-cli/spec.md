...
`AGENTS.md`, `.agents/skills/`, and `Development/` workflow immediately.
...
- Do not refresh product documentation in `Documentation/`.
...
3. The command installs:
   - `AGENTS.md`
   - `.agents/skills/development-initiative-context/SKILL.md`
   - `Development/`
   - `Documentation/` unless `--no-documentation` is supplied
...
- [x] Installing with `--no-documentation` skips `Documentation/`.
