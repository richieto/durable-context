...
When `context/cycles/*/initiatives/*/release-doc-notes.md` exists, it can make
the refresh sharper by pointing at behavior that should become docs later.
Those notes are hints, not authority; verify them against the tag diff.
Reference pages remain self-contained after disposable context is deleted.
