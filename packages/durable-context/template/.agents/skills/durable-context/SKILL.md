...
2. Require an initiative name. If none was supplied, list lifecycle-managed
   initiatives whose condition is Active, Paused, or Blocked, list legacy
   folders separately, and ask the human to name one or create a new one. Never
   infer the initiative from the branch name.
3. If the work is still small, recommend the agent's native planning behavior.
   Invoking this router alone is not that second choice: create durable context
   only if the human explicitly chooses it after the recommendation.
4. For a new initiative, use the project-owned `_templates/initiative/README.md`
   ...
   two files into `context/initiatives/<slug>/` and replace placeholders. Never
   overwrite the project-owned template to upgrade it.
5. If an existing initiative README lacks the lifecycle markers, report it as
...
4. Continue work authorized by the user's request until the next human choice,
   ...
   - Plan Review: offer `devils-advocate`; it remains advisory until a material
     challenge exists.
